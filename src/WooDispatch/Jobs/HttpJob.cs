...
using WooDispatch.Model;
using WooDispatch.Service;

namespace WooDispatch.Jobs
{
    ...
    public class HttpJob : IJob
    {
        ...
        public HttpJob(IHttpService _httpService)
        {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                loginfo.Result = $"<span class='result'>{MaxLeft(result,1000)}</span>";
                if (!response.IsSuccessStatusCode)
                {
                    loginfo.ErrorMsg = $"<span class='error'>{MaxLeft(result, 3000)}</span>";
                    //await ErrorAsync(loginfo.JobName, new Exception(MaxLeft(result, 3000)), JsonConvert.SerializeObject(loginfo), mailMessage);
        ...
        /// <returns></returns>
        public string MaxLeft(string str, int len)
        {
            if (str == null)
